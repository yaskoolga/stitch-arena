...
export default function ProjectDetailPage() {
  ...
  const username = params.username.replace('@', ''); // Remove @ from URL
  ...
  const { data: project, isLoading } = useQuery<Project>({
    ...
    queryFn: () => fetch(`/api/projects/by-slug?username=${encodeURIComponent(username)}&slug=${encodeURIComponent(slug)}`).then((r) => r.json()),
    enabled: !!session,
  });

  // Get project ID for backwards compatibility with existing API calls
  ...
  const deleteProject = useMutation({
    mutationFn: () => fetch(`/api/projects/${project?.id}`, { method: "DELETE" }).then((r) => r.json()),
    onSuccess: () => {
  ...
  const handleQuickPhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const logRes = await fetch(`/api/projects/${project?.id}/logs`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="relative z-10">
          ...
          <div className="flex gap-2 flex-wrap mb-5">
            {project.isPublic && session?.user?.id !== project.userId && (
              <>
                <LikeButton projectId={project.id} variant="outline" className="rounded-full" />
                <FollowProjectButton projectId={project.id} projectOwnerId={project.userId} variant="outline" showCount className="rounded-full" />
              </>
            )}
            {session?.user?.id === project.userId && (
              <Link href={`/projects/\${projectId}/logs/new`}>
      ...
      {project.isPublic && (
        <CommentsSection projectId={project.id} projectOwnerId={project.userId} />
      )}
